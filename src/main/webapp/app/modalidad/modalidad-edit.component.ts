import { Component, inject, OnInit } from '@angular/core';
import { CommonModule } from '@angular/common';
import { ActivatedRoute, Router, RouterLink } from '@angular/router';
import { ReactiveFormsModule, FormControl, FormGroup, Validators } from '@angular/forms';
import { InputRowComponent } from 'app/common/input-row/input-row.component';
import { ModalidadService } from 'app/modalidad/modalidad.service';
import { ModalidadDTO } from 'app/modalidad/modalidad.model';
import { ErrorHandler } from 'app/common/error-handler.injectable';
import { updateForm } from 'app/common/utils';


@Component({
  selector: 'app-modalidad-edit',
  imports: [CommonModule, RouterLink, ReactiveFormsModule, InputRowComponent],
  templateUrl: './modalidad-edit.component.html'
})
export class ModalidadEditComponent implements OnInit {

  modalidadService = inject(ModalidadService);
  route = inject(ActivatedRoute);
  router = inject(Router);
  errorHandler = inject(ErrorHandler);

  currentIdModalidad?: number;

  editForm = new FormGroup({
    idModalidad: new FormControl({ value: null, disabled: true }),
    idUsuReg: new FormControl(null, [Validators.required]),
    idUsuMod: new FormControl(null),
    nombre: new FormControl(null, [Validators.maxLength(50)])
  }, { updateOn: 'submit' });

  getMessage(key: string, details?: any) {
    const messages: Record<string, string> = {
      updated: $localize`:@@modalidad.update.success:Modalidad was updated successfully.`
    };
    return messages[key];
  }

  ngOnInit() {
    this.currentIdModalidad = +this.route.snapshot.params['idModalidad'];
    this.modalidadService.getModalidad(this.currentIdModalidad!)
        .subscribe({
          next: (data) => updateForm(this.editForm, data),
          error: (error) => this.errorHandler.handleServerError(error.error)
        });
  }

  handleSubmit() {
    window.scrollTo(0, 0);
    this.editForm.markAllAsTouched();
    if (!this.editForm.valid) {
      return;
    }
    const data = new ModalidadDTO(this.editForm.value);
    this.modalidadService.updateModalidad(this.currentIdModalidad!, data)
        .subscribe({
          next: () => this.router.navigate(['/modalidads'], {
            state: {
              msgSuccess: this.getMessage('updated')
            }
          }),
          error: (error) => this.errorHandler.handleServerError(error.error, this.editForm, this.getMessage)
        });
  }

}
